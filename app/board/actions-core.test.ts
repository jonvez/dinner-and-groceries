 ...
 *
 * Lazy week creation moved to `lib/week/open-week.ts` (shared with /grocery) and
 * is covered by `lib/week/open-week.test.ts`.
 */
...
function makeClient(results: Record<string, Result>) {
  ...
  return {
    client: { from } as unknown as Parameters<typeof proposeNewDish>[0],
    calls: { fromTables, dishesInsert, proposalsInsert },
  };
}
